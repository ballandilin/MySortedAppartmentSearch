...
class QuickStartApp:
    ...
    def display_available_files(self):
        ...
        if not self.data_files:
            ...
            print("Veuillez d'abord lancer le scraping ou placer des fichiers de données dans le répertoire.")
            return False
        ...
        for i, file_info in enumerate(self.data_files, 1):
            ...
            print(f"   Modifié le: {file_info['modified'].strftime('%Y-%m-%d %H:%M:%S')}")
            print()
    ...
    def select_data_file(self):
        ...
        try:
            choice = input(f"Choisissez un fichier (1-{len(self.data_files)}): ").strip()
...
class QuickImmoApp(tk.Tk):
    ...
    def create_treeview(self, parent):
        ...
        # Colonnes
        columns = ("Prix", "Type", "Localisation", "Surface", "Pièces", "Équipements", "Colocation", "Studio")
    ...
    def populate_data(self):
        ...
        self.populate_tree(self.rejected_tree, self.sorted_data.get('rejected', {}))
    ...
    def populate_tree(self, tree, data):
        ...
        for key, item in data.items():
            ...
            # Surface et pièces
            surface = f"{item.get('surface_m2', 'N/A')} m²" if item.get('surface_m2') else 'N/A'
            pieces = f"{item.get('nombre_pieces', 'N/A')}" if item.get('nombre_pieces') else 'N/A'
            
            # Équipements depuis spécificités
            specificites = item.get('specificite', [])
            equipements = ', '.join([s for s in specificites if 'Étage' not in s and 'm²' not in s and 'pièce' not in s][:3])
