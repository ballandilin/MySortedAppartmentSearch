...
class SortScrapSearch:
    ...
    def convertSeLogerApiFormat(self, item):
        ...
        try:
            ...
            prix_str = f"{item.get('price', 0)} €" if item.get('price') else "N/A"
            ...
            colocation_keywords = ["colocation", "coloc", "Colocation", "Coloc", "colocataire", "colocation"]
            studio_keywords = ["studio", "Studio", "STUDIO"]
            
            colocation = self.contains_word(description, colocation_keywords) or rooms <= 1
            studio = self.contains_word(description, studio_keywords) or (rooms == 1 and bedrooms == 0)
    ...
    def sortSearch(self):
        ...
        for key, item in self.search.items():
            ...
            has_valid_price = self.validatePrice(item.get("prix", item.get("price", "")))
    ...
    def validatePrice(self, prix_str):
        ...
        try:
            ...
            if isinstance(prix_str, (int, float)):
                ...
                import re
                prix_match = re.search(r'(\d+(?:\.\d+)?)', str(prix_str).replace(" ", "").replace(",", "."))
                if prix_match:
    ...
    def validateSurface(self, specificites):
        ...
        if isinstance(specificites, list):
            for spec in specificites:
                if "m²" in str(spec):
                    surface_match = re.search(r'(\d+(?:\.\d+)?)\s*m²', str(spec))
                    if surface_match:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if export_choice in ['o', 'oui', 'y', 'yes']:
            filename = input("Nom du fichier (défaut: resultats_tries.json): ").strip()
            if not filename:
